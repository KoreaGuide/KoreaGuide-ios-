        ...
        func asURLRequest() throws -> URLRequest {
            let result: (path: String, parameters: Parameters, method: HTTPMethod, headers: HTTPHeaders) = {
                ...
                case let .register(nickName, email, password, level):
                    return("/api/user", ["nickName": nickName, "email": email, "pwd":password, "level":level], .post, defaultHeaders)
                case let .login(email, password):
                    return ("/api/user/login",["email": email, "pwd":password], .post, defaultHeaders)
                case let .emailCheck(email):
                    return ("/api/user/checkDuplicate",["email": email], .post, defaultHeaders)
        ...
        .responseJSON { response in
          ...
          /*do {
            ...
            //if let accessToken = response.response?.allHeaderFields["Authorization"] as? String {
              ...
              UserDefaults.ID = jwt.body["id"] as? Int
